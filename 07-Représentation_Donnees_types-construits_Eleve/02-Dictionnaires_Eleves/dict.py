...
def augmente(prixAugmente: dict, pourcentageAugmentation: float or int):
    for el in prix:
        prixAugmente[el] *= 1 + pourcentageAugmentation/100
    return prixAugmente
...
def maxi(listeObjets: dict):
    print(max(listeObjets))

# def ote(commande: dict, stock: dict):
    # for el in commande:
        # if commande[el] > stock[el]:
            # stock[el] -= commande[el]
        # else: 
            # 
            # stock[el] = 0
